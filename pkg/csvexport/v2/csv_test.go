...
import (
	...
	"github.com/spring-media/curation-pkgs-public/pkg/csvexport/v2"
...
func (d mockScan) Scan(ctx context.Context, opt v2.ScanOption, startKey map[string]types.AttributeValue) ([]map[string]interface{}, map[string]types.AttributeValue, error) {
	return d.resp, map[string]types.AttributeValue{}, nil
}

func TestDynamoToCSVV2(t *testing.T) {
	t.Parallel()
	option := v2.ScanOption{}

	db := mockScan{resp: dynamoMockResp}
	b, err := v2.DynamoToCSV(db, context.Background(), option)
...
func TestDynamoToCSVWithCols(t *testing.T) {
	...
	db := mockScan{resp: dynamoMockResp}
	cols := v2.Columns{
		v2.Column{Name: "ArticleLastUpdated"},
		v2.Column{Name: "Block", OverwriteValue: true, OverwriteWithValue: ""},
		v2.Column{Name: "Categories", OverwriteValue: true, OverwriteWithValue: nil},
		v2.Column{Name: "EntriesAveragedFromHome", OverwriteValue: true, OverwriteWithValue: 3},
		v2.Column{Name: "IsPremium", OverwriteValue: true, OverwriteWithValue: 1.33},
		v2.Column{Name: "IsSponsored", OverwriteValue: true, OverwriteWithValue: false},
		v2.Column{Name: "PerformanceLastUpdated"},
	}
	b, err := v2.DynamoToCSV(db, context.Background(), v2.ScanOption{}, v2.WithColumns(cols))
...
func TestDynamoToCSVWithColsTargetName(t *testing.T) {
	...
	cols := v2.Columns{
		v2.Column{Name: "ArticleLastUpdated"},
		v2.Column{Name: "PerformanceLastUpdated", TargetName: "PerformanceUpdatedLast"},
	}
	b, err := v2.DynamoToCSV(db, context.Background(), v2.ScanOption{}, v2.WithColumns(cols))
...
func TestDynamoToCSVWithColsValueFunc(t *testing.T) {
	...
	cols := v2.Columns{
		v2.Column{Name: "ArticleLastUpdated"},
		v2.Column{Name: "Block", ValueFunc: valueFn},
		v2.Column{Name: "NewBlock", ValueFunc: valueFnNew, ValueFuncCol: "Block"},
	}
	b, err := v2.DynamoToCSV(db, context.Background(), v2.ScanOption{}, v2.WithColumns(cols))
